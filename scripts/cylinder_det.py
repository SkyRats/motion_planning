...
class cylinder_detector():

    def clear_cylinder(self, dist, index):
        temp = []
        indices_temp = []
        temp.append(dist)
    ...
    def get_cylinder_candidates(self, sensor):
        ...
        if len(dists) > 0:
            ...
            while i < len(dists):
                ...
                # Nesse caso, seria chao, entao reinicializa as listas temporarias
                    ...
                    (temp, indices_temp) = self.clear_cylinder(dists[i], indices[i])
    ...
    def get_cylinders_xy_and_radius(self, sensor):
        objects, objects_indices = self.get_cylinder_candidates(sensor)
        cylinders = []
        for i in range(len(objects)):
            cx, cy, r = self.find_circle_properties(objects[i], objects_indices[i])
            if self.radii_are_similar(CYLINDER_RADIUS, r):
                cylinders.append( Cylinder(cx, cy, r) )
        return cylinders

    def find_circle_properties(self, obj, obj_indices):
        cx_average = cy_average = r_average = 0

        if len(obj) > 3:
            thetas = self.indices_to_angles(obj_indices)

            final_index = len(obj)-1 
            x1, y1 = self.polar_to_cartesian(obj[0], thetas[0])
            x2, y2 = self.polar_to_cartesian(obj[final_index], thetas[final_index])
            ...
            while 0 < i < final_index:
                ...
                x3, y3 = self.polar_to_cartesian(obj[i], thetas[i])
                cx, cy, r = self.find_circle_xy_and_radius(x1, y1, x2, y2, x3, y3)
                ...
                if (self.centres_are_close(cx, cy, cx_average, cy_average)
                    and self.radii_are_similar(r, r_average)):
    ...
    def find_circle_xy_and_radius(self, x1, y1, x2, y2, x3, y3):
        # Source
...
if __name__ == '__main__':
    ...
    cyl = cylinder_detector()
    while not rospy.is_shutdown():
        rospy.wait_for_message('/laser/scan', LaserScan)
        cylinders = cyl.get_cylinders_xy_and_radius(laser_data.ranges)
        print(len(cylinders))
