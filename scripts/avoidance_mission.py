    ...
    if len(dists) > 0:
        ...
        while i < len(dists):
            ...
            # Nesse caso, seria chao, entao reinicializa as listas temporarias
                ...
                if 0 < len(temp) < prop_max*res or np.average(temp) < 0.7:   # Se nenhum dos de cima, entao   um cilindro. Ai o codigo bota nas listas "objetos" e "obj_indices" e reinicializa os temporarios
        ...
        if sensor[-1] != 0 and indices[0] == 0:
            ...
            if 0 < len(temp) < prop_max*res or np.average(temp) < 0.7:
...
def run():
    ...
    while not rospy.is_shutdown() and d > 0.3:
        ...
        for i in range(len(objects)):
            ...
            Fi = Kr * a/(laser_range - b) + c if laser_range > 0.67 else MAX_VEL
            Fix = -Fi*np.cos(theta)
            Fiy = -Fi*np.sin(theta)
            Ft += np.array([Fix, Fiy])
